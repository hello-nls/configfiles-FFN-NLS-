...
' 		Status Codes
			...
			Public Const LOAN_STATUS_BK_NOTICE = 40 '41 
			Public Const LOAN_STATUS_BK_FILED = 41 '42 
			Public Const LOAN_STATUS_BK_ACTIVE = 41 '42 later replaced LOAN_STATUS_BK_FILED
			Public Const LOAN_STATUS_BK_DISMISSED = 42 '43 
			Public Const LOAN_STATUS_BK_COMPLETED = 43 '44  
			Public Const LOAN_STATUS_BK_DISCHARGE7 = 47 
			Public Const LOAN_STATUS_BK_DISCHARGE11 = 48  
			Public Const LOAN_STATUS_BK_DISCHARGE12 = 49  
			Public Const LOAN_STATUS_BK_DISCHARGE13 = 50  
...
'		Statuses                                                       
			...
			Public Const TEMPLATE_STATUS_BK_NOTICE = 14
			...
			Public Const TEMPLATE_STATUS_BK_COMPLETED = 17           
...
' * Document Templates
			Public Const DOCTEMPL_FPLUS_DEBT_VALIDATION_LETTER = 11 
			Public Const DOCTEMPL_CPLUS_DEBT_VALIDATION_LETTER = 10
'Other Constants
    'Security
			Public Const SECURITY_GROUP_MODIFICATION_APPROVERS = 22
			Public Const SECURITY_GROUP_COLLECTION_OWNERS = 23
   'Collection campaigns
			Public Const COLLECTION_OWNERS_CAMPAIGN_SETUPNO = 55
    'Folders and Directories
			Public Const INVESTOR_TRANSFERS_PENDING_DIR = "\\tsclient\T\FreedomPlus Servicing\Transfers\PendingTransfers"
			Public Const INVESTOR_TRANSFERS_COMPLETED_DIR = "\\tsclient\T\FreedomPlus Servicing\Transfers\CompletedTransfers\"
...
'key - [class2 codenum],[class2 name],[F+ ach company ID],[F+ ach company name],[C+ ach company ID],[C+ ach company name]
	...
	investormatrix(38) = "38,26-STONERIDGE- ALITI,44,StoneRidge - ALITI F+,45,StoneRidge - ALITI C+"
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'GLOBAL SQL STATEMENT (Variable)
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

PENDING_COMPLETE_CLOSE_LOAN_SQL = "Select                                                                                                                       " & chr(13) & _
"--top 10                                                                                                                                                                        " & chr(13) & _
"   --XML output                                                                                                                                                                 " & chr(13) & _
"   '<NLS>' +                                                                                                                                                                    " & chr(13) & _
"   Case When l.current_suspense > 0 Then                                                                                                                                        " & chr(13) & _
"   --When the loan has balance in Suspence, include this transaction, otherwise skip.                                                                                           " & chr(13) & _
"     '<TRANSACTIONS>' +                                                                                                                                                         " & chr(13) & _
"     '<TRANSACTIONCODE TransactionCode=""462"" EffectiveDate=""' + convert(char(8),getdate(),1) +                                                                               " & chr(13) & _
"     '"" LoanNumber=""' + l.loan_number + '"" Amount=""' + cast(convert(numeric(10,2),round(l.current_suspense,2)) as varchar) + '"" />' +                                      " & chr(13) & _
"     '</TRANSACTIONS>' Else '' End +                                                                                                                                            " & chr(13) & _
"                                                                                                                                                                                " & chr(13) & _
"   --Change loan status to CLOSED, COMPLETED                                                                                                                                    " & chr(13) & _
"   '<LOAN UpdateFlag=""1"" LoanNumber=""' + l.loan_number + '"" LoanStatusCode=""CLOSED"" ClosedDate=""'  + convert(char(8),getdate(),1) + '"" >' +                             " & chr(13) & _
"   '<LOANSTATUSES Operation=""DELETE"" LoanStatusCode=""PENDING COMPLETE"" EffectiveDate=""' + convert(char(8),getdate(),1) + '""/>'+                                           " & chr(13) & _
"   '<LOANSTATUSES Operation=""ADD"" LoanStatusCode=""COMPLETED"" EffectiveDate=""' + convert(char(8),getdate(),1) + '""/></LOAN></NLS>',                                        " & chr(13) & _
"   --Other loan data                                                                                                                                                            " & chr(13) & _
"  convert(varchar(15), l.acctrefno),                                                                                                                                            " & chr(13) & _
"   --use loanaccte portfolio code id to determine doc template #                                                                                                                " & chr(13) & _
"   'template' = CASE When l.portfolio_code_id = 0 then 5  When l.portfolio_code_id = 1 then 6 End,                                                                              " & chr(13) & _
"                                                                                                                                                                                " & chr(13) & _
"   --use loan Mail paper statement flag  OR missing email address to determine if zero balnance document is sent to queue for printing (TRUE)  or if it will be emailed (FALSE)." & chr(13) & _
"   'SendToQueue' = CASE When                                                                                                                                                    " & chr(13) & _
...
"	c.email as emailtoaddr,                                                                                                                                                      " & chr(13) & _
"	c.firstname1+' '+lastname1 as name,l.loan_number,c.firstname1,+lastname1                                                           " & chr(13) & _
"   --ls.effective_date,l.status_code_no,current_suspense, l.current_payoff_balance,dateadd(dd,+14,ls.effective_date)                                                            " & chr(13) & _
"                                                                                                                                                                                " & chr(13) & _
"from                                                                                                                                                                            " & chr(13) & _
...
"   and l.acctrefno NOT IN (SELECT  distinct                                     /*Filter to remove loans that have had a comment and a letter/email generated */                " & chr(13) & _
"                              acctrefno                                         /*the current day already. This prevents duplicate mailings the same day.     */                " & chr(13) & _
"                           FROM loanacct_comments                               /*This check will reset each day allowing mailing the next day    */                            " & chr(13) & _
"                           where comment_description like '%ZERO BALANCE LETTER'                                                                                                " & chr(13) & _
"                             and left(created, 11) = left(getdate(),11)                                                                                                         " & chr(13) & _
"						   )                                                                                                                                                     " & chr(13) & _
"--and  l.acctrefno = 99999999      /*temporary shut off*/                                                                                                                       " & chr(13) & _
"--and l.current_suspense >= 0                                                                                                                                                   " & chr(13) & _
"                                                                                                                                                                                " & chr(13) & _
